...
from signal_based_ee import BertForSequenceClassification
...
def process_current_batch(args,outputs):
    ...
    for layer,highway_exit in enumerate(all_highway_exits):
        ...
        if args.signal_type=="labelscore":
            indices = ((highway_signal_copy >= 0) & (highway_signal_copy > args.early_exit_threshold))
        elif args.signal_type=="entropy" or args.signal_type=="energy":
            indices = ((highway_signal_copy >= 0) & (highway_signal_copy < args.early_exit_threshold))


        logits_current_batch[indices, :] = highway_logits[indices, :]
        exit_layer_counter[layer + 1] += torch.sum(indices).item()  # update the exit_layer_counter
...
def evaluate(args, model, tokenizer):
    ...
    for eval_task in eval_task_names:   #eval on each task
        ...
        logger.info("  {}-{}-{} ".format(args.signal_type,args.temperature,args.early_exit_threshold))
        ...
        eval_time = time.time() - st    #calculate the inference time
